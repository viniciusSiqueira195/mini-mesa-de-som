...
A versão **1.2.5** melhora a continuidade do áudio, adiciona intensidade
ajustável à redução de ruído e um modo de compatibilidade para o modificador de
voz. Também inclui atalho global para mostrar ou minimizar a janela, opção de
iniciar com o Windows e impede que duas instâncias sejam abertas ao mesmo tempo.

## Instalação rápida

Baixe `MiniMesaDeSom-Setup-1.2.5.exe` na página da
...
`installer-output\MiniMesaDeSom-Setup-1.2.5.exe`. O aplicativo é empacotado em
