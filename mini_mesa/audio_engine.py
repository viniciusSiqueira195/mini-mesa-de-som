    ...
    def _process_audio(self, indata, frames: int):
        ...
        with self._processing_lock:
            ...
            if soundboard is not None:
                soundboard_audio = soundboard.render(frames)
                if soundboard_audio is not None:
                    if self._soundboard_settings.ducking_enabled:
                        voice_level = float(
                            self._np.sqrt(self._np.mean(mono_input * mono_input))
                        )
                        activity = min(1.0, voice_level / 0.06)
                        soundboard_duck -= (
                            self._soundboard_settings.ducking_percent / 100.0
                        ) * activity
            ...
            if soundboard_audio is not None:
                stereo += (soundboard_audio * soundboard_duck)[:, self._np.newaxis]
